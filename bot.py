...
async def receipt_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not state or state.get("action") != "awaiting_receipt":
...
async def handle_admin_decision(query, context, data):
    ...
    user_text = (
        ...
        f"👤 یوزرنیم: `{service_username}`\n\n"
        f"🔗 لینک اشتراک (Subscription) — این رو تو اپلیکیشن V2Ray/WireGuard خودت وارد کن:\n"
        f"`{config_link}`\n\n"
        f"این لینک همه‌ی سرورهای موجود رو خودکار براتون آپدیت می‌کنه."
    )
    try:
        await context.bot.send_message(chat_id=order["user_id"], text=user_text, parse_mode="Markdown")
    except Exception as e:
