...
HELP_TEXT = (
    ...
    "☎️ برای پشتیبانی بیشتر: @Summertime221_py"
)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        text = "برای پشتیبانی با ادمین در ارتباط باش: @Summertime221_py"
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 بازگشت", callback_data="main_menu")]])
        await query.edit_message_text(text, reply_markup=keyboard)
...
# ============================================================
# FREE TRIAL (500MB / 1 day, one per Telegram account)
# ============================================================
async def handle_free_trial(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # NOTE: the panel's own 'test' flag is fixed at 150MB/1day per the docs, and the panel's
    # documented minimum for a real (non-test) service is 0.5 gig / 1 day — 0.25 was rejected
    # with "Minimum gig: 0.5". So the smallest trial we can offer is 500MB/1day. This draws a
    # small amount from the reseller's own panel wallet balance, not from the customer's bot wallet.
    result = api_create_service(0.5, 1, test=0)
    ...
    add_service(user_id, service_username, "تست رایگان (500MB/1روز)", expiry_time=expiry_time)
    mark_free_trial_used(user_id)

    await update.message.reply_text("🎁 سرویس تست رایگانت (۵۰۰ مگابایت / ۱ روز) ساخته شد!")
